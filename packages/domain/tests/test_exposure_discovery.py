...
def test_pinned_requirements_are_matched_without_inventing_dependency_provenance() -> None:
    source = CapturedOsvSource(
        {
            "results": [
                {
                    "vulns": [
                        {
                            ...
                            "affected": [
                                {
                                    "package": {"ecosystem": "PyPI", "name": "flat-pkg"},
                                    "versions": ["1.0"],
                                }
                            ],
                        }
    ...
    package = PackageInstance(
        ...
        version="1.0",
        direct=None,
        source=PackageSource((("manifest", "requirements.txt"),)),
        dependency_paths=None,
    )

    result = ExposureDiscovery(source).discover(snapshot_with(package))

    assert source.queries == (OsvPackageQuery(name="flat-pkg", version="1.0"),)
    assert result.exposures[0].ranking.direct_dependency is None
    assert result.exposures[0].ranking.dependency_depth is None
    assert result.exposures[0].ranking.score == 0
